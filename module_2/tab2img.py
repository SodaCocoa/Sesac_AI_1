...
class CustomDataset(Dataset):
    def __init__(self, data, labels, transform=None):
        ...
        self.converter = Tab2Img()  # Tab2Img 인스턴스 생성

    def __len__(self):
        return len(self.data)

    def __getitem__(self, idx):
        imglike_data = self.converter.transform(self.data[idx].reshape(1, -1))  # 데이터를 이미지로 변환
        imglike_data = torch.tensor(imglike_data, dtype=torch.float32).permute(2, 0, 1)  # 채널 차원을 앞으로 이동
...
if __name__ == '__main__':
    ...
    parameters = {
        'batch_size': [8, 16, 32],
        'lr': [0.00025, 0.0001, 0.001, 0.01],
        'tuning_rate': [0.2, 0.4, 0.6]
    }
